...
(
    ...
    `startAt`     timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '比赛开始',
    `createAt`    timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '开始时间',
...
## 数据初始化
```sql
insert into tb_match values(0,'测试比赛1',now(),now(),now(),1);
```
